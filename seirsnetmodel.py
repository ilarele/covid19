...
class SEIRSNetworkModel():
    ...
    def __init__(self,
        ...
        self.numS[0] = self.numNodes - self.numE[0] - self.numI[
            0] - self.numD_E[0] - self.numD_I[0] - self.numR[0] - self.numF[0]
        self.N[0] = self.numS[0] + self.numE[0] + self.numI[0] + self.numD_E[
            0] + self.numD_I[0] + self.numR[0]
        ...
        self.X = numpy.array([self.S] * int(self.numS[0]) +
                             ...
                             [self.F] * int(self.numF[0])).reshape(
                                 (self.numNodes, 1))
        numpy.random.shuffle(self.X)

        self.store_Xseries = store_Xseries
        if (store_Xseries):
            self.Xseries = numpy.zeros(shape=(5 * self.numNodes,
                                              self.numNodes),
                                       dtype='uint8')
        ...
        self.transitions = {
            ...
            'DEtoDI': {
                'currentState': self.D_E,
                'newState': self.D_I
            },
            'DItoR': {
                'currentState': self.D_I,
                'newState': self.R
            },
            'DItoF': {
                'currentState': self.D_I,
        ...
        if (node_groups):
            ...
            for groupName, nodeList in node_groups.items():
                ...
                self.nodeGroupData[groupName]['numI'] = numpy.zeros(
                    5 * self.numNodes)
                self.nodeGroupData[groupName]['numD_E'] = numpy.zeros(
                    5 * self.numNodes)
                self.nodeGroupData[groupName]['numD_I'] = numpy.zeros(
                    5 * self.numNodes)
                self.nodeGroupData[groupName]['numR'] = numpy.zeros(
                    5 * self.numNodes)
                self.nodeGroupData[groupName]['numF'] = numpy.zeros(
                    5 * self.numNodes)
                self.nodeGroupData[groupName]['N'] = numpy.zeros(5 *
                                                                 self.numNodes)
                ...
                self.nodeGroupData[groupName]['N'][0] = self.nodeGroupData[
                    groupName]['numS'][0] + self.nodeGroupData[groupName][
                        'numE'][0] + self.nodeGroupData[groupName]['numI'][
                            0] + self.nodeGroupData[groupName]['numD_E'][
                                0] + self.nodeGroupData[groupName]['numD_I'][
                                    0] + self.nodeGroupData[groupName]['numR'][
                                        0]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def update_parameters(self):
            ...
            (self.numNodes,
             1)) if isinstance(self.parameters['mu_0'],
                               (list, numpy.ndarray)) else numpy.full(
                                   ...
                                   shape=(self.numNodes, 1))
        self.mu_H = self.parameters['mu_H']
        self.mu_ICU = self.parameters['mu_ICU']
        ...
        self.beta_D = (
            numpy.array(self.parameters['beta_D']).reshape(
                (self.numNodes, 1)) if isinstance(self.parameters['beta_D'],
                                                  (list, numpy.ndarray)) else
            numpy.full(fill_value=self.parameters['beta_D'],
                       shape=(self.numNodes, 1))
        ) if self.parameters['beta_D'] is not None else self.beta
        self.sigma_D = (
            numpy.array(self.parameters['sigma_D']).reshape(
                (self.numNodes, 1)) if isinstance(self.parameters['sigma_D'],
                                                  (list, numpy.ndarray)) else
            numpy.full(fill_value=self.parameters['sigma_D'],
                       shape=(self.numNodes, 1))
        ) if self.parameters['sigma_D'] is not None else self.sigma
        self.gamma_D = (
            numpy.array(self.parameters['gamma_D']).reshape(
                (self.numNodes, 1)) if isinstance(self.parameters['gamma_D'],
                                                  (list, numpy.ndarray)) else
            numpy.full(fill_value=self.parameters['gamma_D'],
                       shape=(self.numNodes, 1))
        ) if self.parameters['gamma_D'] is not None else self.gamma
        self.mu_D = (numpy.array(self.parameters['mu_D']).reshape(
            (self.numNodes, 1)) if isinstance(self.parameters['mu_D'],
                                              (list, numpy.ndarray)) else
                     numpy.full(fill_value=self.parameters['mu_D'],
                                shape=(self.numNodes, 1))
                     ) if self.parameters['mu_D'] is not None else self.mu_I
        ...
        if (self.parameters['beta_local'] is not None):
            if (isinstance(self.parameters['beta_local'],
                           (list, numpy.ndarray))):
                if (isinstance(self.parameters['beta_local'], list)):
                    self.beta_local = numpy.array(
        ...
        if (self.parameters['beta_D_local'] is not None):
            if (isinstance(self.parameters['beta_D_local'],
                           (list, numpy.ndarray))):
                if (isinstance(self.parameters['beta_D_local'], list)):
                    self.beta_D_local = numpy.array(
                        self.parameters['beta_D_local'])
                else:  # is numpy.ndarray
        ...
        if (self.beta_local.ndim == 1):
            self.A_beta = scipy.sparse.csr_matrix.multiply(
                self.A, numpy.tile(self.beta_local,
        ...
        if (self.beta_D_local.ndim == 1):
            self.A_Q_beta_D = scipy.sparse.csr_matrix.multiply(
                self.A_Q, numpy.tile(self.beta_D_local,
                                     (1, self.numNodes))).tocsr()
        elif (self.beta_D_local.ndim == 2):
            self.A_Q_beta_D = scipy.sparse.csr_matrix.multiply(
                self.A_Q, self.beta_D_local).tocsr()
    ...
    def update_G(self, new_G):
        ...
        if type(new_G) == numpy.ndarray:
            ...
            self.A = networkx.adj_matrix(
                new_G)  # adj_matrix gives scipy.sparse csr_matrix
        else:
            raise BaseException(
    ...
    def update_Q(self, new_Q):
        ...
        if type(new_Q) == numpy.ndarray:
            ...
            self.A_Q = networkx.adj_matrix(
                new_Q)  # adj_matrix gives scipy.sparse csr_matrix
        else:
            raise BaseException(
                "Input an adjacency matrix or networkx object only.")
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def update_scenario_flags(self):
        self.testing_scenario = (
            (numpy.any(self.psi_I) and
             (numpy.any(self.theta_I) or numpy.any(self.phi_I)))
            or (numpy.any(self.psi_E) and
                (numpy.any(self.theta_E) or numpy.any(self.phi_E))))
        self.tracing_scenario = (
            (numpy.any(self.psi_E) and numpy.any(self.phi_E))
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def calc_propensities(self):
        ...
        if (self.testing_scenario and numpy.any(self.numD_I[self.tidx])
                and numpy.any(self.beta_D)):
            transmissionTerms_DI = numpy.asarray(
                scipy.sparse.csr_matrix.dot(self.A_Q_beta_D,
                                            self.X == self.D_I))

        numContacts_D = numpy.zeros(shape=(self.numNodes, 1))
        if (self.tracing_scenario and (numpy.any(self.numD_E[self.tidx])
                                       or numpy.any(self.numD_I[self.tidx]))):
            numContacts_D = numpy.asarray(
                scipy.sparse.csr_matrix.dot(self.A, ((self.X == self.D_E) |
                                                     (self.X == self.D_I))))

        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        propensities_StoE = (self.p * (
            (self.beta * self.numI[self.tidx] + self.q * self.beta_D *
             self.numD_I[self.tidx]) / self.N[self.tidx]) +
                             (1 - self.p) * numpy.divide(
                                 (transmissionTerms_I + transmissionTerms_DI),
                                 self.degree,
                                 out=numpy.zeros_like(self.degree),
                                 where=self.degree != 0)) * (self.X == self.S)
        ...
        propensities = numpy.hstack([
            ...
            propensities_ItoF, propensities_EtoDE, propensities_ItoDI,
            propensities_DEtoDI, propensities_DItoR, propensities_DItoF,
            propensities_RtoS, propensities__toS
        ])

        columns = [
            'StoE', 'EtoI', 'ItoR', 'ItoF', 'EtoDE', 'ItoDI', 'DEtoDI',
            'DItoR', 'DItoF', 'RtoS', '_toS'
        ]
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def increase_data_series_length(self):
        self.tseries = numpy.pad(self.tseries, [(0, 5 * self.numNodes)],
                                 mode='constant',
                                 constant_values=0)
        self.numS = numpy.pad(self.numS, [(0, 5 * self.numNodes)],
                              mode='constant',
                              constant_values=0)
        self.numE = numpy.pad(self.numE, [(0, 5 * self.numNodes)],
                              mode='constant',
                              constant_values=0)
        self.numI = numpy.pad(self.numI, [(0, 5 * self.numNodes)],
                              mode='constant',
                              constant_values=0)
        self.numD_E = numpy.pad(self.numD_E, [(0, 5 * self.numNodes)],
                                mode='constant',
                                constant_values=0)
        self.numD_I = numpy.pad(self.numD_I, [(0, 5 * self.numNodes)],
                                mode='constant',
                                constant_values=0)
        self.numR = numpy.pad(self.numR, [(0, 5 * self.numNodes)],
                              mode='constant',
                              constant_values=0)
        self.numF = numpy.pad(self.numF, [(0, 5 * self.numNodes)],
                              mode='constant',
                              constant_values=0)
        self.numH = numpy.pad(self.numH, [(0, 5 * self.numNodes)],
                              mode='constant',
                              constant_values=0)
        self.N = numpy.pad(self.N, [(0, 5 * self.numNodes)],
                           mode='constant',
                           constant_values=0)

        if (self.store_Xseries):
            self.Xseries = numpy.pad(self.Xseries, [(0, 5 * self.numNodes),
                                                    (0, 0)],
                                     mode=constant,
                                     constant_values=0)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def run_iteration(self):
        ...
        if (propensities.sum() <= 0.0):
            print("propensities.sum()", propensities.sum())
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        assert (self.X[transitionNode] == self.transitions[transitionType]
                ['currentState'] and self.X[transitionNode] != self.F
        ...
        self.numD_E[self.tidx] = numpy.clip(
            numpy.count_nonzero(self.X == self.D_E),
            a_min=0,
            a_max=self.numNodes)
        self.numD_I[self.tidx] = numpy.clip(
            numpy.count_nonzero(self.X == self.D_I),
            a_min=0,
            a_max=self.numNodes)
        ...
        self.numH[self.tidx] = self.mu_H * self.numI[self.tidx]

        self.N[self.tidx] = numpy.clip(
            (self.numS[self.tidx] + self.numE[self.tidx] +
             self.numI[self.tidx] + self.numD_E[self.tidx] +
             self.numD_I[self.tidx] + self.numR[self.tidx]),
            a_min=0,
            a_max=self.numNodes)
        ...
        if (self.t >= self.tmax or
            (self.numI[self.tidx] < 1 and self.numE[self.tidx] < 1
             and self.numD_E[self.tidx] < 1 and self.numD_I[self.tidx] < 1)):
            self.finalize_data_series()
            if (self.numI[self.tidx] < 1 and self.numE[self.tidx] < 1
             and self.numD_E[self.tidx] < 1 and self.numD_I[self.tidx] < 1):
                print("numI, numE, numD_E, numD_I < 1")
            return False
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self,
        ...
        Dseries = (self.numD_E +
                   self.numD_I) / self.numNodes if plot_percentages else (
        ...
        Hseries = Iseries.copy() * self.mu_ICU
        ICU_series = Hseries.copy() * self.mu_ICU
        
        if not plot_percentages:
            ICU_series *= REAL_POPULATION/self.numNodes
            Hseries *= REAL_POPULATION/self.numNodes

        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Draw the reference data:
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if plot_H and plot_ICU:
            label_prefix = "Hospitalized + ICU"
        elif plot_H:
            label_prefix = "Hospitalized"
        elif plot_ICU:
            label_prefix = "ICU"
        ...
        if (dashed_reference_results):
            ...
            stack_values = dashed_reference_results.numH
            if plot_ICU:
                stack_values += dashed_reference_results.numH.copy(
                ) * self.mu_ICU
            if not plot_H:
                stack_values -= dashed_reference_results.numH

            dashedReference_stack = stack_values[::int(self.numNodes / 100)] / (
                self.numNodes if plot_percentages else 1)

            ax.plot(dashedReference_tseries,
                    dashedReference_stack,
                    color='#E0E0E0',
                    linestyle='--',
                    label='%s (%s)' % (label_prefix, dashed_reference_label),
                    zorder=2)

        if (shaded_reference_results):
            shadedReference_tseries = shaded_reference_results.tseries
            stack_values = shaded_reference_results.numH
            if plot_ICU:
                stack_values += shaded_reference_results.numH.copy(
                ) * self.mu_ICU
            if not plot_H:
                stack_values -= shaded_reference_results.numH

            shadedReference_stack = (stack_values) / (self.numNodes if
                                                      plot_percentages else 1)

            ax.fill_between(shaded_reference_results.tseries,
                            shadedReference_stack,
                            0,
                            color='#EFEFEF',
                            label='%s (%s)' %
                            (label_prefix, shaded_reference_label),
                            zorder=0)
        ...
        if (any(Eseries) and plot_E == 'line'):
            ax.plot(numpy.ma.masked_where(Eseries <= 0, self.tseries),
                    ...
                    zorder=6)
        if (combine_D and
            (any(Dseries) and plot_D_E == 'line' and plot_D_I == 'line')):
            ax.plot(numpy.ma.masked_where(Dseries <= 0, self.tseries),
                    numpy.ma.masked_where(Dseries <= 0, Dseries),
                    color=color_D_E,
                    label='$D_{all}$',
        ...
        if plot_ICU:
            ...
            if (plot_percentages):
                y_plot = ICU_available_beds / REAL_POPULATION
            else:
                y_plot = ICU_available_beds
            ax.axhline(y=y_plot,
                       color="#FF0099",
                       linestyle=":",
                       alpha=1,
                       label="[RO] ICU_available_beds: %d" % ICU_available_beds)

        active_infections = 7084
        if (plot_percentages):
            y_plot = active_infections/ REAL_POPULATION
        else:
            y_plot = active_infections
        ax.axhline(y=y_plot,
                    color="orange",
                    linestyle=":",
                    alpha=1,
                    label="[RO] Active infections: %d" % active_infections)

        if plot_H:
            Total_available_beds = 40000
            if (plot_percentages):
                plot_y = Total_available_beds / REAL_POPULATION
            else:
                plot_y = Total_available_beds
            ax.axhline(y=plot_y,
                       color=color_H,
                       linestyle=":",
                       alpha=1,
                       label="[RO] Total_available_beds: %d" % Total_available_beds)
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Draw the plot labels:
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ax.set_xlabel('Days', fontsize=14)
        ax.set_ylabel('percent of population'
                      if plot_percentages else 'Number of Individuals', fontsize=14)
